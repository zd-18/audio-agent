...
class IncrementalSha256 {
  ...
  private finish() {
    ...
    const highBits = Math.floor(this.bytesHashed / 0x20000000) >>> 0
...
interface Sha256FileOptions {
  signal?: AbortSignal
  readSize?: number
  onProgress?: (percent: number) => void
}

export async function sha256File(file: Blob, options: Sha256FileOptions = {}) {
  const hasher = new IncrementalSha256()
  const readSize = options.readSize ?? 4 * 1024 * 1024
  let offset = 0
  while (offset < file.size) {
    if (options.signal?.aborted) throw new DOMException('摘要计算已暂停', 'AbortError')
    const end = Math.min(file.size, offset + readSize)
  ...
  if (file.size === 0) options.onProgress?.(100)
